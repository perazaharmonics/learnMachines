...
class LogisticRegressionGD:
    ...
    errors_ : list
        Mean Square Error loss function values in each epoch.
    ...
    def fit(self, X, y):
        ...
        Returns
        -------
        self : Instance of Logic RegressionGD
        ...
        for _ in range(self.n_iter):
            ...
            loss = (-y.dot(np.log(output)) 
                    - ((1-y).dot(np.log(1-output)))\
                         / X.shape[0] )  # Compute Log-Likelihood Function
            self.losses_.append(loss)  # Store the Log-likelihood Function result
